...
```
jq -s -f filters/cargo-geiger/normalize.jq geiger.json
```
...
```
STRIP_PATHS=1 cargo geiger --output-format Json | jq -f filters/cargo-geiger/normalize.jq
```
...
```
jq -s -f filters/cargo-geiger/identity.jq geiger.json
```
