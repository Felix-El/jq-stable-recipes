...
Full normalization: produces stable JSON from non-deterministic deny output. Drops `log` events (timestamps vary every run). Keeps all `diagnostic` fields (code, severity, message, graphs, labels, notes) and the `summary` event. Normalizes internal array ordering within each event. Sorts diagnostics deterministically by `(code, message, primary-crate name, version)`. Recursively sorts all object keys.
